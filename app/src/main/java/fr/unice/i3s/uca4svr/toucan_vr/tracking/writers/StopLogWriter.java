...
public class StopLogWriter extends LogWriter {
  ...
  public StopLogWriter(String logFilePrefix) {
    super(logFilePrefix, "fr.unice.i3s.uca4svr.tracking.writers.StopLogWriter"
      + loggerNextID++);
  }
  ...
  protected String createLogFileName(String logFilePrefix) {
    ...
    Date date = new Date();
    return String.format("%s_stop_%s.csv", logFilePrefix, dateFormat.format(date));
  }
}
